...
$offlisting
$ondelim
$include A_Inputs%ds%inputs%ds%variability%ds%d_szn.csv
$offdelim
$onlisting
/ ;
...
Parameter cost_dropped_load "--Rs/MWh-- Cost for dropped load" ;
*Use the maximum cost and increase it by Rs1 so that it is always higher than the highest generator cost
cost_dropped_load = smax((i,v,r), cost(i,v,r) ) + 1 ;

Parameter cost_flow "--Rs/MW-- Cost for using transmission lines" ;
...
* Set a lower bound on generation from dispatchable hydro
  gen_low(d,hr,i,v,r)$[(hydro_d(i) )$cap_max_in(i,v,r)] = cap_max_model(d,i,v,r) * sum{szn$d_szn(d,szn), mingenfrac(i,r,szn) } ;

* Calculate the amount of energy that would be supplied by dispatchable hydro each day given the availability
  hydro_gen_day(d,r) = sum{(i,v)$[hydro_d(i) ], cap_max_in(i,v,r) * avail_day(d,i,v,r) * card(hr) } ;
...
if(Sw_Stor = 0,
  cap_max_model(d,i,v,r)$storage(i) = 0 ;
) ;
...
cost(i,v,r)$[cap_max_feas(i,v,r)$storage(i)$(cost(i,v,r) = 0)] = cost_flow ;
...
* optional energy constraint or daily CF constraint for gas... uncomment to enable
* eq_gasct_max_cf(r)      "--unitless-- Gas CT generation cannot exceed 20% daily capacity factor"
...
eq_gen_cap(hr,i,v,r)$cap_max_feas(i,v,r)..
    ...
    GEN(hr,i,v,r) + STORAGE_IN(hr,i,v,r)$[storage(i)$Sw_Stor]
...
eq_load_balance(hr,r)..
    ...
    - sum{(i,v)$[storage(i)$cap_max_feas(i,v,r)$Sw_Stor], STORAGE_IN(hr,i,v,r) }
...
eq_hydro_balance(r)$[hydro_gen_feas(r)$Sw_Hydro]..
   ...
   sum{(hr,i,v)$[(hydro_d(i))$cap_max_feas(i,v,r)], GEN(hr,i,v,r) }
...
*  sum{(v,i)$[sameas(i,'CT-GAS')], cap_max(i,v,r) * 24 * 0.2}

*  =g=

*  sum{(hr,v,i)$[sameas(i,'CT-GAS')], GEN(hr,i,v,r)}
...
eq_storage_level(hr,i,v,r)$[storage(i)$cap_max_feas(i,v,r)$Sw_Stor]..
    ...
    - GEN(hr,i,v,r)$[storage(i)$cap_max_feas(i,v,r)]
;

eq_storage_duration(hr,i,v,r)$[storage(i)$cap_max_feas(i,v,r)$Sw_Stor]..
