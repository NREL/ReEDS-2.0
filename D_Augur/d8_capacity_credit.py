...
#%%
    ...
    cap_frac_cspns =    pd.DataFrame(columns = ['i','r','rs','cspns_frac'])
    ...
    dt_map = pd.read_csv(os.path.join('A_Inputs','inputs','variability', 'h_dt_szn.csv'))
    if not mult_year:
        idx = dt_map['year'] == args['year']
    ...
    peaking_stor_all = pd.DataFrame(
        columns=['region', 'season', 'duration', 't', 'MW'])
    timeslice_hours_all = pd.DataFrame(
        columns=['h', 'region', 'season', '%TopHrs'])

#%%  FOR LOOP FOR CCREGS

    # March through this for all CCREGs
    for ccreg in hierarchy['region'].drop_duplicates():
        ...
        resources_ccreg = resources[resources['region'] == ccreg].reset_index(
            drop=True)
        resources_ccreg = resources_ccreg.rename(columns={'area':'r','tech':'i'})
        ...
        cap_stor_agg_ccreg = cap_stor_agg[
            cap_stor_agg['region'] == ccreg].reset_index(
                drop=True)
        cap_stor_ccreg = cap_stor[cap_stor['r'].isin(
            hierarchy[hierarchy['region'] == ccreg]['r'])].reset_index(
                drop=True)
        try:
...
#%%  LOOP FOR SEASONS
        ...
        for season in seasons:
            ...
            all_cc = get_vg_cc(
                resources_ccreg, next_year, cc_vg_results['cap_useful_MW'], cap_frac_cspns,
                hierarchy, ccreg, all_cc, season)
...
#%%  WRITE DATA TO GDX FILE
    ...
    peaking_stor_all.columns = ['region', 'szn', 'bin', 't', 'MW']
    timeslice_hours_all.columns = ['h', 'region', 'szn', 'value']
...
#%%
    ...
    # Find the limit of 1-hour storage capacity
...
#%%
    temp_cap_frac = cap_frac_cspns[cap_frac_cspns['r'].isin(
        hierarchy[hierarchy['region'] == ccreg]['r'])]
...
#%%
    all_rs = hierarchy[hierarchy['region'] == ccreg]['rs'].drop_duplicates(
        ).tolist()
    for rs in all_rs:
...
#    df = peaking_stor_all.copy()
    ...
    results['region'] = ccreg
    results['t'] = str(next_year)
    if not season == 'year':
        results['season'] = season
        results = results[['region', 'season', 'duration', 't', 'MW']].copy()
        results.loc[len(results), :] = [ccreg, season, safety_bin,
                                        str(next_year), safety_bin_size]
    else:
        results = results[['region', 'duration', 't', 'MW']]
