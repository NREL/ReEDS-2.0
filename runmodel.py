...
def setupEnvironment(ui_input=None):
	...
	for c in to_run:
		...
		path_entries = ['yearset','HourlyLoadFile','PRMregion_file','PRMstate_file','FuelLimit_file','FuelPrice_file','TechCost_file','MinLoad_file','Hours_file','Load_file','PeakDemRegion_file','PeakDemState_file','IVT_file','Trancap_file','InterTrancost_file', 'AbsGrowLim_file', 'CapNonRsc_file', 'CapRsc_file', 'PrescribedNonRsc_file', 'PrescribedRsc_file', 'CfOut_file', 'RscDat_file']
...
def runModel(caseindex,options,caseSwitches,lstfile,niter,timetype,yearfile,INPUTDir,
	...
	with open(os.path.join(OutputDir, 'compile_' + lstfile + FILE_EXTENSION), 'w') as OPATH:
		OPATH.write("gams " + str(os.path.join("A_Inputs", "a_inputs_old.gms")) +\
		...
		with open(os.path.join(OutputDir, 'run_' + lstfile + FILE_EXTENSION), 'w') as OPATH:
			...
			for i in range(startiter,niter):
				...
				if i < niter and niter > 1:
					if cc_curtchoice == 1:
						OPATH.writelines(f"{os.getenv('PYTHON_PATH')} " + str(os.path.join("D_Augur","d00_pickle_prep_v2.py")) + " " + hourlyloadfile + " " + lstfile +\
										 " " + OutputDir + '\n')
