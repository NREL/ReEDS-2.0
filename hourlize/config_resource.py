...
reg_col = 'cnty_fips' #region column from supply curve file (sc_path). 'cnty_fips' for county, could be 'model_region' if it is already in the supply curve.
reg_out_col = 'reeds_ba' #Either (1) the same value as 'reg_col' to use a column from the sc file or (2) a column from reg_map_path (mapped via 'reg_col'; must be unique in reg_map_path). 
                         #Typically either 'reeds_ba' for ReEDS regions or 'cnty_fips' for county-level supply curves.
