...
Run use "run_hourlize.py" -- see README for setup and details.
"""
...
# copies reV config json files to supply curve metadata folder in ReEDS 
...
# resource classes and used by writesupplycurves.py to create bins based on supply curve cost.
...
def add_cost(df_sc, tech, upv_type, cost_out, reg_out_col):
    ...
    if cost_out == 'combined_eos_trans':
    ...
    elif cost_out == 'combined_off_ons_trans':
        ...
        df_sc['supply_curve_cost_per_mw'] = df_sc[cost_out]
...
def copy_outputs(outpath, reedspath, sc_file, sc_path, casename, reg_out_col,
    ...
    if copy_to_reeds:
        if reg_out_col == "cnty_fips" or "county" in casename:
            print("""County-level supply curves are not kept in the repo due to their size 
...
if __name__== '__main__':
    ...
    #%% Add cost
    df_sc = add_cost(df_sc, cf.tech, cf.upv_type, cf.cost_out, cf.reg_out_col)
