...
def ProcessGdx():
    ...
    # Co-located Green Hydrogen tech curtailment
    gen_h2 = gdxin['GEN'].loc[gdxin['GEN']['i'].isin(['WIND_H2','UPV_H2','DISTPV_H2'])].drop(columns={'Marginal','Lower','Upper','Scale'})
    gen_h2 = summarize(gen_h2, 'Level', ['i','r','h','t','scenario']).rename(columns={'Level':'Gen'})
    cap_h2 = gdxin['CAP'].loc[gdxin['CAP']['i'].isin(['WIND_H2','UPV_H2','DISTPV_H2'])].drop(columns={'Marginal','Lower','Upper','Scale'})
    cap_h2 = summarize(cap_h2, 'Level', ['i','r','t','scenario'])
    cap_h2 = pd.merge(gdxin['m_cf'], cap_h2, on = ['i','r','scenario'])
    cap_h2['potential'] = cap_h2['Value'] * cap_h2['Level']
    cap_h2 = map_rs_to_state(cap_h2, rmap)
    cap_h2 = summarize(cap_h2, 'potential', ['i','r','h','t','scenario'])
    curt_h2 = pd.merge(cap_h2, gen_h2, on = ['i', 'r', 'h', 't', 'scenario'], how = 'left')
    curt_h2['curtailment'] = curt_h2['potential'] - curt_h2['Gen'].fillna(0)
    curt_h2 = map_tech_to_type(curt_h2, 'i')
    curt_h2 = summarize(curt_h2, ['potential','curtailment'], ['Type','h','t','scenario', 'r'], drop=False)
    curt_h2['curt_h2_frac'] = curt_h2['curtailment'] / curt_h2['potential']
 ...
 #   storlev = setnames(storlev, 'STOR_LVL_MWh')
  ...
  #  emit = gdxin['EMIT']
    ...
    # curt_h2
    curt_h2_tslc = map_h_to_tsname(curt_h2, 'h')
    curt_h2_tslc.drop(columns=['h',0], inplace=True)
    curt_h2_tslc.rename(columns={'t':'Year','Type':'Technology', 'r': 'st', 'potential': 'Potential Generation (MW)', 'Gen': 'Generation (MW)', 'curtailment': 'H2 Solar PV and H2 Wind: Export to Grid (MW)'}, inplace=True)
    curt_h2_tslc = sorting(curt_h2_tslc, True, True, False)

    return annual_out, firmcap, tslc_out, tx_out, dem, dem_tslc, peakdem_prm, costs, costs_diff, curt_tslc, curt_h2_tslc, cap_rs, cap_diff, gen_diff, costs_state
#%%

def write_outputs(dir):
    annual_out, firmcap, tslc_out, tx_out, dem, dem_tslc, peakdem_prm, costs, costs_diff, curt_tslc, curt_h2_tslc, cap_rs, cap_diff, gen_diff, costs_state = ProcessGdx()
   ...
   # Path(os.path.join(csvsdir,'BAs')).mkdir(parents=True, exist_ok=True)
    ...
    # curt_h2_timeslice.csv
    curt_h2_tslc.rename(columns={'Year':'year'}, inplace=True)
    curt_h2_tslc['time'] = curt_h2_tslc['time'].astype(str)
    curt_h2_tslc['time'] = curt_h2_tslc['time'].replace('peak', 'national peak')
    curt_h2_tslc['tech'] = 'Curtailment'
    curt_h2_tslc['tech'] = curt_h2_tslc['tech'].replace('Curtailment', 'H2 Solar PV and H2 Wind: Export to Grid')
    curt_h2_tslc['H2 Solar PV and H2 Wind: Export to Grid (MWh)'] = curt_h2_tslc['hours'] * curt_h2_tslc['H2 Solar PV and H2 Wind: Export to Grid (MW)']
    ##gen['Technology'] = gen['Technology'].replace('Imports', 'Imports (Nepal & Bhutan)')
    curt_h2_tslc.to_csv(os.path.join(csvsdir, 'curt_h2_timeslice.csv'), index=False)
    ...
    # curt_h2_frac.csv
    curt_h2_tslc['Technology'] = curt_h2_tslc['Technology'].astype('object')
    curt_h2_frac = summarize(curt_h2_tslc, ['Potential Generation (MW)','H2 Solar PV and H2 Wind: Export to Grid (MW)', 'H2 Solar PV and H2 Wind: Export to Grid (MWh)'], ['Technology','year', 'st', 'scenario'], drop=False)
    curt_h2_frac.rename(columns={'Technology':'tech'}, inplace=True)
    curt_h2_frac['curt_frac'] = curt_h2_frac['H2 Solar PV and H2 Wind: Export to Grid (MW)'] / curt_h2_frac['Potential Generation (MW)']
    curt_h2_frac['tech'] = 'H2 Solar PV and H2 Wind: Export to Grid'
    curt_h2_frac.to_csv(os.path.join(csvsdir,'curt_h2_frac.csv'), index=False)
    ...
    # curt_h2_diff.csv
    curt_h2_diff = curt_h2_frac.pivot_table(index=['st','tech','year'], columns='scenario', values='H2 Solar PV and H2 Wind: Export to Grid (MWh)').reset_index()
    for i in scenarios[1:]:
        curt_h2_diff[i] = curt_h2_diff[i].fillna(0) - curt_h2_diff[scenarios[0]].fillna(0)
    if len(scenarios) == 1:
        curt_h2_diff[scenarios[0]] = 0
    else:
        curt_h2_diff.drop(scenarios[0], axis=1, inplace=True)
    curt_h2_diff = curt_h2_diff.set_index(['st','tech','year']).stack().reset_index(name='Difference (MWh)')
    curt_h2_diff.to_csv(os.path.join(csvsdir,'curt_h2_diff.csv'), index=False)
    ...
    shutil.copyfile('vizit-config_UP_h2.json', os.path.join(csvsdir, 'vizit-config_UP_h2.json'))
