...
gdxfiles = args[1]
runs = gsub("output_|.gdx","", gdxfiles)
gdxpaths = file.path('E_Outputs','runs', runs, "outputs", gdxfiles)
...
sdbin_cap[, season := factor(season, levels = unique(season),
                             labels = c('Dec - Jan','Feb - Mar','Apr - Jun','Jul - Sep','Oct - Nov'))]
...
if('i1' %in% names(cap_iter)){
  setnames(cap_iter,c('i1','i2','i3','i4','i5'),c('i','v','rold','t','cciter'))
}else{
  setnames(cap_iter,'r','rold')
}
cap_iter = merge(cap_iter, r.rs.map, by.x = 'rold', by.y = 'rs', all.x = T)
cap_iter[is.na(r), r := rold]
if('value' %in% names(cap_iter)){
  cap_iter = cap_iter[,.(value = sum(value)), by = .(i,v,r,t,cciter,scenario)]
}else{
...
firm_cap = rbind(firm_conv, firm_hydro, fill = TRUE)
firm_cap = rbind(firm_cap, firm_vg, fill = TRUE)
if((!anyNA(firm_stor)) & length(names(firm_stor)) == 6){
  firm_cap = rbind(firm_cap, firm_stor, fill = TRUE)
}
...
#------------------------------#
# CC for dispatchable hydro plants ----
#------------------------------#
pull_gdx_data(type = 'param', gdx.name = 'cc_hydro', r.name = 'cc_hydro')

col.names = c('Technology','State','season','Year','value','scenario')
re_name_cols(cc_hydro)

cc_hydro[, season := factor(season, levels = unique(season),
                            labels = c('Dec - Jan','Feb - Mar','Apr - Jun','Jul - Sep','Oct - Nov'))]

#------------------------------#
# CC for storage and renewables ----
#------------------------------#

# wind and solar CC by resource region
...
cc_int = merge(cc_int, r.rs.map, by = 'rs', all.x = T)
setnames(cc_int, 'r','State')
cc_int[,rs := as.character(rs)]

cc_int[, season := factor(season, levels = unique(season),
                          labels = c('Dec - Jan','Feb - Mar','Apr - Jun','Jul - Sep','Oct - Nov'))]
