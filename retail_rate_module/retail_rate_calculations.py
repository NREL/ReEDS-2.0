...
def main(run_dir, inputpath='inputs.csv', write=True, verbose=0):
    ...
    # run_dir = os.path.expanduser('~/github/ReEDS-2.0/runs/v20210722_mainM0_Ref/')
    ...
    #%% Start calculations
    # Load regions
    regions_map = pd.read_csv(os.path.join(run_dir, 'inputs_case', 'regions.csv')).rename(columns={'p':'r'})
    ...
    load_rt = (
        pd.read_csv(os.path.join(run_dir, 'outputs', 'load_rt.csv'))
        .rename(columns={'Dim1':'r', 'Dim2':'t', 'Val':'busbar_load'}))
    ...
    system_costs = pd.read_csv(os.path.join(run_dir, 'outputs', 'systemcost_ba.csv'))
    ...
    # for separately in 'op_trans' (using FERC data instead of ReEDS)
                     if (('op_' in i) and (i != 'op_transmission_fom'))]
    ...
    # Ingest the capital cost multipliers. This includes the construction interest multiplier 
    # and regional capital cost multipliers, but not other adjustments like the ITC or depreciation. 
    try:
        cap_cost_mult = pd.read_csv(
            os.path.join(run_dir, 'inputs_case', 'retail_cap_cost_mult.csv.gz'))
    except FileNotFoundError:
        try:
            cap_cost_mult = pd.read_pickle(
                os.path.join(run_dir, 'inputs_case', 'retail_cap_cost_mult.pkl'))
        except ValueError:
            import pickle5
            with open(os.path.join(run_dir, 'inputs_case', 'retail_cap_cost_mult.pkl'), 'rb') as f:
                cap_cost_mult = pickle5.load(f)
    cap_cost_mult.rename(columns={'r':'region'}, inplace=True)
    ...
    rsc_dat = rsc_dat.rename(
        columns={'Dim1':'r', 'Dim2':'s', 'Dim3':'i', 'Dim4':'bin', 'Dim5':'dat_type', 'Val':'dat_value'})
    ...
    try:
        eval_period = pd.read_csv(
            os.path.join(run_dir, 'inputs_case', 'retail_eval_period.csv.gz'))
    except FileNotFoundError:
        try:
            eval_period = pd.read_pickle(
                os.path.join(run_dir, 'inputs_case', 'retail_eval_period.pkl'))
        except ValueError:
            with open(os.path.join(run_dir, 'inputs_case', 'retail_eval_period.pkl'), 'rb') as f:
                eval_period = pickle5.load(f)
    eval_period.rename(columns={'r':'region'}, inplace=True)

    try:
        depreciation_sch = pd.read_csv(
            os.path.join(run_dir,'inputs_case','retail_depreciation_sch.csv.gz'))
    except FileNotFoundError:
        try:
            depreciation_sch = pd.read_pickle(
                os.path.join(run_dir,'inputs_case','retail_depreciation_sch.pkl'))
        except ValueError:
            with open(os.path.join(run_dir,'inputs_case','retail_depreciation_sch.pkl'), 'rb') as f:
                depreciation_sch = pickle5.load(f)
    ...
    nongen_cost_cats = pd.DataFrame(
        ...
        data = [['inv_investment_spurline_costs_rsc_technologies', 'cap_spurline'],
                ...
                ['inv_converter_costs', 'cap_interconn'],
    ...
    # within ReEDS. 
    nongen_capex = nongen_capex.rename(columns={'cost_type':'i', 'cost':'capex'})

    # Merge on the cost_cat - just a shorter name for columns used elsewhere
    nongen_capex = nongen_capex.merge(nongen_cost_cats, on='i', how='left')

    # Add to the main df_capex
    df_capex = pd.concat([df_capex, nongen_capex], sort=False).reset_index(drop=True)


    #%% Estimate historical capital expenditures for existing transmission capacity, 
    # allocate to states, and add to df_capex
    init_trans_cap = pd.read_csv(
        os.path.join(run_dir, 'inputs_case', 'transmission_capacity_initial.csv'))
    init_trans_cap = init_trans_cap.rename(columns={'AC':'AC_MW', 'DC':'DC_MW'})

    trans_dist = (
        pd.read_csv(
            os.path.join(run_dir, 'inputs_case', 'transmission_distance.csv'),
            header=0, names=['r','rr','trtype','distance'])
        .pivot(index=['r','rr'], columns='trtype', values='distance')
        .fillna(0).reset_index().rename(columns={'AC':'AC_mile', 'DC':'DC_mile'})
    )

    ### Drop duplicate values associated with NARIS
    ## First assign empty AC_mile to DC_mile
    trans_dist.loc[trans_dist.duplicated(subset=['r','rr'])&trans_dist.AC_mile.isnull(),'AC_mile'] = (
        trans_dist.loc[trans_dist.duplicated(subset=['r','rr'])&trans_dist.AC_mile.isnull(),'DC_mile'])
    ## Drop the duplicate entries
    trans_dist.drop(
        trans_dist.loc[
            trans_dist.duplicated(subset=['r','rr'],keep=False) & trans_dist.DC_mile.isnull()
        ].index, inplace=True)
    ### Double-check that there are no duplicate entries
    ### (Do this instead of drop_duplicates because if there are errors,
    ###  we want to make sure we keep the right entries)
    if len(trans_dist.loc[trans_dist.duplicated(subset=['r','rr'])]) > 0:
        raise Exception("Duplicate entries in transmission_distance.csv")

    init_trans_cap = init_trans_cap.merge(
        trans_dist[['r', 'rr', 'AC_mile', 'DC_mile']], on=['r', 'rr'], how='left') 
    init_trans_cap[['AC_mile', 'DC_mile']] = init_trans_cap[['AC_mile', 'DC_mile']]#.fillna(100.0)

    init_trans_cap['AC_MW-miles'] = init_trans_cap['AC_MW'] * init_trans_cap['AC_mile']
    init_trans_cap['DC_MW-miles'] = init_trans_cap['DC_MW'] * init_trans_cap['DC_mile']

    ## Use .fillna(0) here since lines with no DC have DC_MW-miles == nan
    init_trans_cap['MW-miles'] = (
        init_trans_cap['AC_MW-miles'].fillna(0) + init_trans_cap['DC_MW-miles'].fillna(0))

    line_cost = pd.read_csv(os.path.join(run_dir, 'inputs_case', 'transmission_line_cost.csv'),
                    header=0, names=['r','line_cost_per_mw_mile_AC','line_cost_per_mw_mile_DC'])

    init_trans_capex_lump = (
        init_trans_cap[['r', 'rr', 'AC_MW-miles', 'DC_MW-miles']]
        .merge(line_cost.rename(columns={'line_cost_per_mw_mile_AC':'line_cost_per_mw_mile_AC_r',
                                         'line_cost_per_mw_mile_DC':'line_cost_per_mw_mile_DC_r'}), 
            on='r', how='left')
        .merge(line_cost.rename(columns={'line_cost_per_mw_mile_AC':'line_cost_per_mw_mile_AC_rr',
                                         'line_cost_per_mw_mile_DC':'line_cost_per_mw_mile_DC_rr',
                                         'r':'rr',}), 
            on='rr', how='left')
    )
    init_trans_capex_lump['line_cost_per_mw_mile_AC'] = (
        init_trans_capex_lump['line_cost_per_mw_mile_AC_r'] 
        + init_trans_capex_lump['line_cost_per_mw_mile_AC_rr']) / 2
    init_trans_capex_lump['line_cost_per_mw_mile_DC'] = (
        init_trans_capex_lump['line_cost_per_mw_mile_DC_r'] 
        + init_trans_capex_lump['line_cost_per_mw_mile_DC_rr']) / 2
    init_trans_capex_lump['init_trans_capex'] = (
        init_trans_capex_lump['AC_MW-miles'] * init_trans_capex_lump['line_cost_per_mw_mile_AC']
        + init_trans_capex_lump['DC_MW-miles'] * init_trans_capex_lump['line_cost_per_mw_mile_DC']
    )

    init_trans_capex_lump_by_r = pd.concat(
        [init_trans_capex_lump[['r', 'init_trans_capex']],
        init_trans_capex_lump[['rr', 'init_trans_capex']].rename(columns={'rr':'r'})
        ], ignore_index=True, sort=False)
    init_trans_capex_lump_by_r['init_trans_capex'] = init_trans_capex_lump_by_r['init_trans_capex'] / 2

    init_trans_capex_lump_by_r['i'] = 'inv_transmission_line_investment'
    ...
    trans_capex_reeds = (
        nongen_capex.groupby(['state','t'])['capex'].sum()
    ...
    trans_cap = (
        ...
        .rename(columns={'Dim1':'r', 'Dim2':'rr', 'Dim3':'t', 'Dim4':'type', 'Val':'tran_cap'})
        .merge(trans_dist[['r', 'rr', 'AC_mile', 'DC_mile']], on=['r', 'rr'], how='left') 
    )
    trans_cap['dist'] = np.where(trans_cap['type']=='AC', trans_cap['AC_mile'], trans_cap['DC_mile'])
    trans_cap['state1'] = trans_cap['r'].map(reedsregion2state)
    trans_cap['state2'] = trans_cap['rr'].map(reedsregion2state)

    ### Check to make sure there are no null distances
    if any(trans_cap.dist.isnull().values):
        raise Exception(
            'Missing transmission distances; '
            'check outputs/tran_out.csv and inputs_case/transmission_distance.csv')

    trans_cap['MW-mile'] = trans_cap['tran_cap'] * trans_cap['dist']

    ### Assign each line to one region. 
    ### We get two rows for each line, so we need to divide by 2 when we use trans_om later.
    trans_om = pd.concat(
        [trans_cap[['r', 't', 'MW-mile']], 
        trans_cap[['rr', 't', 'MW-mile']].rename(columns={'rr':'r'})], 
        ignore_index=True, sort=False)
    ...
    if input_daproj['aggregation'] == 'nation':
        ...
        trans_df = (
            ...
            .merge(trans_cap.groupby('t')['MW-mile'].sum(), on=['t'], how='left').dropna()
        ...
        trans_df['trans_om_per_mw_mile'] = trans_df['trans_om'] / trans_df['MW-mile']
        ...
        trans_om['op_trans'] = trans_om['MW-mile'] * trans_om['trans_om_per_mw_mile'] / 2
    ...
    ### State-specific O&M/MWh and O&M/MW-mile
        ...
        #     .merge(trans_om.groupby(['t','state'])['MW-mile'].sum()/2, 
    ...
    df_capital_costs['adit'] = (
        (df_capital_costs['tax_dep_f_cum'] - df_capital_costs['accounting_dep_f_cum']) 
        * df_capital_costs['capex_inf_adj'] * df_capital_costs['tax_rate'] )
    ...
    df_capital_costs['income_tax'] = (
        df_capital_costs['equity_return'] / (1.0 - df_capital_costs['tax_rate']) 
        - df_capital_costs['equity_return'])
    ...
    # There is also a row for each year that a [i,c] combo was able to be built, but we can't
    #   apply each value to the full [i,c] generation each year. Very crudely just dividing
    ...
    itc_value_distributed['t'] = itc_value_distributed['t_online'] + itc_value_distributed['itc_year']
    ...
    # for each year of an asset's life. 
    ...
    # Group by [t, state]
    itc_value_grouped = (
        itc_value_distributed[['t', 'state', 'itc_normalized_value']]
        .groupby(['t', 'state'], as_index=False).sum())

    # National value, for reference
    itc_value_nat = (
        itc_value_distributed[['itc_normalized_value', 't']]
        .groupby('t', as_index=False).sum())
    itc_value_nat = (
        itc_value_nat.merge(itc_value_df[['t', 'itc_base_value']]
        .groupby('t', as_index=False).sum(), on='t', how='left'))

    # Merge onto main dfall
    dfall = dfall.merge(
        itc_value_grouped[['t', 'state', 'itc_normalized_value']], on=['t', 'state'], how='left')
    dfall['itc_normalized_value'] = dfall['itc_normalized_value'].fillna(0.0)
    ### make it negative because it is a value instead of a cost
    dfall['itc_normalized_value'] = -dfall['itc_normalized_value']
...
#########
#%% PLOTS
def get_dfplot(run_dir, inputpath, plot_dollar_year):
    """
    ...
    ### Convert to ¢/kWh
    for col in [c for c in dfplot.columns if c != 'retail_load']:
...
def post_processing(dfplot):
    ...
    for subcol in subcols:
        ...
        ### Put transmission interconn into transmission
        dfplot[subcol.format('transmission')] += dfplot[subcol.format('interconn')]
        dfplot.drop(subcol.format('interconn'), axis=1, inplace=True)
...
def retail_plots(
    ...
    tracelabels = {
        ...
        'cap_interconn_dep_expense': 'Capital: Transmission intertie depreciation',
        'cap_interconn_debt_interest': 'Capital: Transmission intertie debt interest',
        'cap_interconn_equity_return': 'Capital: Transmission intertie equity return',
        'cap_interconn_income_tax': 'Capital: Transmission intertie income tax',
    }
    ...
    print(
        "The only entries in the following list should be 'ptc_grossup' and 'itc_normalized_value'."
        "\nIf there are additional entries, add them to "
        "\nretail_rate_module/retail_rate_calculations.retail_plots().")
    ...
    for t in range(startyear,endyear+1):
        start = dfplot.loc[t,['ptc_grossup','itc_normalized_value']].sum()
    ...
    ax.set_ylim(
        dfplot[['ptc_grossup','itc_normalized_value']].sum(axis=1).min()*1.5,
...
if __name__ == '__main__':
    ...
    import site
    site.addsitedir(os.path.join(mdir,'..','input_processing'))
    try:
    ...
    run_dir = os.path.join(mdir, '..', 'runs', args.rundir)
