...
from web.constants import (USER_RESET_EMAIL_SUBJECT, USER_SIGNUP_EMAIL_SUBJECT, 
    ...
    REEDS_SIM_INITIATION_SUBJECT, REL_URL_TO_SERVICE_DICT)
...
def rate_limit(func):
    async def inner_wrapper(self, request):
        ...
        if not check_for_throttle(request, REL_URL_TO_SERVICE_DICT.get(request.rel_url, None)):
            logger.info(f"Request accepted for service {request.rel_url} for user {request['userData']['username']}")
            ret_val = await func(self, request)
            return ret_val
        else:
            logger.error(f"Request denied for service {request.rel_url} for user {request['userData']['username']}")
...
def authenticate(func):
    async def inner_wrapper(self, request):
        ...
        try:
            ...
            logger.error(f"Unauthorized request to service {request.rel_url}")
...
class Handler:
    ...
    def __init__(self, loop = None, pool=None):
        ...
        # self.mail = AWS_SES_HTMLMail(os.getenv('AWS_REGION'),os.getenv('AWS_ACCESS_KEY_ID'),os.getenv('AWS_SECRET_ACCESS_KEY'))
        ...
        if authenticated:
            ...
            with session_manager() as session:
                if self.hash.check_password(session, request['userData']['username'], user_detail['oldpassword']):
                    ...
                    self.db.update_password(hash_key, request['userData']['username'])
                    session.commit()
            ...
            return web.json_response(text=json.dumps({"message": "failed"}), status=500)
    ...
    async def handle_signup(self, request):
        ...
        try:
            ...
            if token_is_valid:
                ...
                #self.mail.send_email(os.get_env('REEDS_SENDER'), parsed_data['formdata']['email'], email_body, USER_WELCOME_EMAIL_SUBJECT)
    ...
    async def run_reeds_model(self, request):
        ...
        try:
            ...
            if request['userData']['username'] not in self.instances_dict:
                ...
                # Add to the Queue
            self.simulation_queue.put({'input': reeds_input, 'uuid': reeds_uuid, 'username': request['userData']['username']})
            ...
            email_body = UserSimRunInitiationHTMLMessage().return_rendered_html({
                ...
                'link_to_logs': f"{os.getenv('BASE_URL_FOR_SIMULATION_STATUS')}/{reeds_uuid}"
            })
            print(email_body)

            # TODO: Activate this email during the testing
            #self.mail.send_email(os.get_env('REEDS_SENDER'), usr_email, email_body, REEDS_SIM_INITIATION_SUBJECT)
        ...
        with session_manager() as session:
            ...
            if user:
                ...
                if token != None:
                    ...
                    # TODO: Activate this email during the testing
                    #self.mail.send_email(os.get_env('REEDS_SENDER'), user.email, email_body, USER_RESET_EMAIL_SUBJECT)
        return web.Response(text=json.dumps({
    ...
    async def handle_user_signup_request(self, request):
        ...
        try:
            ...
            # TODO: Activate this email during the testing
            #self.mail.send_email(os.get_env('REEDS_SENDER'), os.get_env('REEDS_SENDER'), email_body, USER_SIGNUP_REQUEST_SUBJECT)
            ...
            try:
                ...
                with session_manager() as session:
                    if self.db.is_admin(session, request['userData']['username']):
                        ...
                        # TODO: Activate this email during the testing
                        #self.mail.send_email(os.get_env('REEDS_SENDER'), data['email'], email_body, USER_SIGNUP_EMAIL_SUBJECT)
            ...
            try:
                ...
                with session_manager() as session:
                    if self.db.is_admin(session, request['userData']['username']):
                        ...
                        # TODO: Activate this email during the testing
                        #self.mail.send_email(os.get_env('REEDS_SENDER'), data['email'], email_body, USER_SIGNUP_EMAIL_REJECT_SUBJECT)
