...
class BaseEmail(ABC):
    ...
    @abstractmethod
    def send_email(
        sender: str, reciever: str, content: str, subject: str
    ) -> bool:
        pass
...
class NologinSMTPTextMail(SMTPMail, BaseEmail):
    def send_email(
        self, sender: str, reciever: str, content: str, subject: str
    ) -> bool:
        stm = SimpleTextMessage()
        with smtplib.SMTP_SSL(
            self.smtp_address, self.smtp_port, context=self.context
        ) as server:
            server.sendmail(
                sender, reciever, stm.prepare_message(subject, content)
            )
...
class NologinSMTPHTMLMail(SMTPMail, BaseEmail):
    def send_email(
        ...
        with smtplib.SMTP_SSL(
            ...
            server.sendmail(
                sender, reciever, htm.prepare_message(subject, content)
            )
...
class SendGridTextMail(BaseSendGrid, BaseEmail):
    def send_email(
        ...
        response = self.sg.client.mail.send.post(
            request_body=sgtm.prepare_message(subject, content)
        )
...
class SendGridHTMLMail(BaseSendGrid, BaseEmail):
    def send_email(
        ...
        response = self.sg.client.mail.send.post(
            request_body=sghm.prepare_message(subject, content)
        )
...
class TestMail(BaseEmail):
    def send_email(
        ...
        print(
            f"Sender {sender}, Reciever {reciever}, content {content}, Subject {subject}"
        )
        return True
...
if __name__ == "__main__":
    ...
    am.send_email(
        "kduwadi2021@gmail.com", "kduwadi2021@gmail.com", message, subject
    )
