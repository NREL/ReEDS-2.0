...
from sqlalchemy import Column, ForeignKey, Integer, String, DateTime, LargeBinary, Boolean, Float
...
class Rules(Base):
    ...
    allowed_requests_per_minute = Column('allowed_requests_per_minute', Float(), nullable=False)
...
class SimulationQueue(Base):
    ...
    created_date = Column('created_date', DateTime(timezone=False),nullable=False)
...
class UserSignupToken(Base):
    ...
    email = Column('email', String(100), ForeignKey("user_signup_request.email"), primary_key=True)
    token = Column('token', String(100), nullable=False)
    token_used = Column('token_used', Boolean, default=False)
    token_created_date = Column('token_created_date', DateTime(timezone=False), default=datetime.datetime.utcnow())
...
class UserSignUpRequest(Base):
    ...
    email = Column('email', String(100), nullable=False, unique=True, primary_key=True)
    ...
    requested_date = Column('requested_date', DateTime(timezone=False), default=datetime.datetime.utcnow())
...
class UserResetToken(Base):
    ...
    username = Column('username', String(100), ForeignKey("users.username"), nullable=False, primary_key=True)
...
class OutputMetadata(Base):
    ...
    author = Column('author', String(100), ForeignKey("users.username"), nullable=False)
    created = Column('created', DateTime(timezone=False), default=datetime.datetime.utcnow)
...
class ScenarioLabels(Base):
    ...
    id = Column('id', Integer, autoincrement=True, primary_key=True)
    user = Column('user', String(100), ForeignKey("users.username"), nullable=False)
...
class PersonnelScenarioState(Base):
    ...
    user = Column('user', String(100), ForeignKey("users.username"), primary_key=True)
    personnel_scen_order = Column('personnel_scen_order', LargeBinary, nullable=False)

class DefaultScenarioState(Base):
    __tablename__ = 'default_scenario_state'
    user = Column('user', String(100), ForeignKey("users.username"), primary_key=True)
    default_scen_order = Column('default_scen_order', LargeBinary, nullable=False)
...
class Users(Base):
    ...
    output_card_state = relationship(OutputCardState, cascade="all, delete-orphan")
    default_scenario_state = relationship(DefaultScenarioState, cascade="all, delete-orphan")
    personnel_scenario_state = relationship(PersonnelScenarioState, cascade="all, delete-orphan")
