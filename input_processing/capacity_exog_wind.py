...
if args.unitdata == 'ABB':
    ...
    raw_curves.rename(columns={'s':'resource_region'}, inplace=True)
    bins = [c for c in raw_curves.columns if c.startswith('bin')]
    ...
    # Prescribed retires
    onshore_ret = prescribed_retire[prescribed_retire['tech']=='wind-ons'].pivot(
        index='resource_region',columns='year',values='cap').fillna(0)
    ...
    raw_curves.drop(bins,1,inplace=True)

    for i in range(0,len(raw_curves),1):
        raw_curves.loc[i,'tech'] = (
            raw_curves.loc[i,'tech'] + '_' + re.findall(r'\d+', raw_curves.loc[i,'class'])[0])
    ...
    wind_onshore[['tech','class','ba','resource_region','year','value']].to_csv(
        os.path.join(outdir,'exog_wind_by_trg.csv'),index=False)
