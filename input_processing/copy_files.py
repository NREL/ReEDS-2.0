...
# reedsdir = os.getcwd()
# reedsdir = os.path.expanduser('~/github2/ReEDS-2.0/')
# inputs_case = os.path.join(reedsdir,'runs','v20230112_spatialM0_ERCOT','inputs_case')
...
runfiles = (
    pd.read_csv(
        os.path.join(reedsdir, 'runfiles.csv'),
        dtype={'fix_cols':str}
    ).fillna({'fix_cols':''})
...
for filepath in nonregionFiles['filepath']:
    if filepath.split('/')[0] in ['inputs', 'postprocessing']:
        ...
        dir_dst = casedir
    src_file = os.path.join(reedsdir, filepath)
...
solveyears = pd.read_csv(
    os.path.join(reedsdir,'inputs','modeledyears.csv'),
...
hierarchy = pd.read_csv(
    os.path.join(reedsdir, 'inputs', 'hierarchy{}.csv'.format(
        '' if (sw['GSw_HierarchyFile'] == 'default')
        else '_'+sw['GSw_HierarchyFile']))
).rename(columns={'*r':'r'})
### Load the rb-to-rs map
rsmap = pd.read_csv(
    os.path.join(reedsdir, 'inputs', 'rsmap_sreg.csv')).rename(columns={'*r':'r'})
...
### If it does not include a '/', then it indicates a column in modeled_regions.csv.
    ...
    modeled_regions = pd.read_csv(
        os.path.join(reedsdir,'inputs','userinput','modeled_regions.csv'))
...
rsmap = rsmap.loc[rsmap['r'].isin(val_r)]
val_r_cap = val_r + rsmap.rs.unique().tolist()

# Export region files
pd.Series(val_r).to_csv(
    os.path.join(inputs_case, 'val_r.csv'), header=False, index=False)
pd.Series(val_r_cap).to_csv(
    os.path.join(inputs_case, 'val_r_cap.csv'), header=False, index=False)

# Create list of CS regions that are included in modeled regions
r_cs = pd.read_csv(os.path.join(reedsdir, 'inputs', 'ctus', 'r_cs.csv'))
...
if int(sw['GSw_IndividualSites']):
    shutil.copy(os.path.join(reedsdir,'inputs','rsmap.csv'),
                inputs_case)
    shutil.copy(os.path.join(reedsdir,'inputs','capacitydata',
                             f'wind-ons_prescribed_builds_site_{sw.GSw_SitingWindOns}.csv'),
                os.path.join(inputs_case,'wind-ons_prescribed_builds.csv'))
    shutil.copy(os.path.join(reedsdir,'inputs','capacitydata',
                             f'wind-ofs_prescribed_builds_site_{sw.GSw_SitingWindOfs}.csv'),
                os.path.join(inputs_case,'wind-ofs_prescribed_builds.csv'))
else:
    # rsmap already loaded in via rsmap_sreg.csv 
    # but has been filtered based on val_r
    rsmap = rsmap.rename(columns={'r':'*r'})
    rsmap.to_csv(os.path.join(inputs_case,'rsmap.csv'),index=False)
    shutil.copy(os.path.join(reedsdir,'inputs','capacitydata',
                             f'wind-ons_prescribed_builds_sreg_{sw.GSw_SitingWindOns}.csv'),
                os.path.join(inputs_case,'wind-ons_prescribed_builds.csv'))
    shutil.copy(os.path.join(reedsdir,'inputs','capacitydata',
                             f'wind-ofs_prescribed_builds_sreg_{sw.GSw_SitingWindOfs}.csv'),
                os.path.join(inputs_case,'wind-ofs_prescribed_builds.csv'))

### Specific versions of files
osprey_num_years = len(sw['osprey_years'].split(','))
shutil.copy(
    os.path.join(
        reedsdir, 'inputs', 'variability', 'index_hr_map_{}.csv'.format(osprey_num_years)),
    os.path.join(inputs_case, 'index_hr_map.csv')
)
shutil.copy(
    os.path.join(
        reedsdir, 'inputs', 'variability', 'd_szn_{}.csv'.format(osprey_num_years)),
    os.path.join(inputs_case, 'd_szn.csv')
)
shutil.copy(
    os.path.join(
        reedsdir,'inputs','state_policies','offshore_req_{}.csv'.format(sw['GSw_OfsWindForceScen'])),
    os.path.join(inputs_case,'offshore_req.csv')
)
shutil.copy(
    os.path.join(
        reedsdir,'inputs','consume',f"dac_gas_{sw['GSw_DAC_Gas_Case']}.csv"),
    os.path.join(inputs_case,'dac_gas.csv')
)
shutil.copy(
    os.path.join(
        reedsdir,'inputs','carbonconstraints',f"capture_rates_{sw['GSw_CCS_Rate']}.csv"),
...
for f in ['distPVCF','distPVcap','distPVCF_hourly']:
    shutil.copy(
        os.path.join(
            reedsdir,'inputs','dGen_Model_Inputs','{s}','{f}_{s}.csv').format(
                f=f, s=sw['distpvscen']),
        os.path.join(inputs_case, f'{f}.csv')
    )
pd.read_csv(
    os.path.join(reedsdir,'inputs','loaddata',f"demand_{sw['demandscen']}.csv"),
...
try:
    ...
    pd.read_csv(
        os.path.join(reedsdir,'inputs','carbonconstraints','methane_leakage_rate.csv'),
        index_col='t',
...
pd.read_csv(
    os.path.join(reedsdir,'inputs','carbonconstraints','ng_crf_penalty.csv'), index_col='t',
)[sw['GSw_NG_CRF_penalty']].rename_axis('*t').to_csv(
    os.path.join(inputs_case,'ng_crf_penalty.csv')
)
pd.read_csv(
    os.path.join(reedsdir,'inputs','carbonconstraints','co2_cap.csv'), index_col='t',
).loc[sw['GSw_AnnualCapScen']].rename_axis('*t').to_csv(
    os.path.join(inputs_case,'co2_cap.csv')
)
pd.read_csv(
    os.path.join(reedsdir,'inputs','carbonconstraints','co2_tax.csv'), index_col='t',
...
pd.read_csv(
    os.path.join(reedsdir,'inputs','national_generation','gen_mandate_trajectory.csv'),
...
pd.read_csv(
    os.path.join(reedsdir,'inputs','national_generation','gen_mandate_tech_list.csv'),
    index_col='*i',
)[sw['GSw_GenMandateList']].to_csv(
    os.path.join(inputs_case,'gen_mandate_tech_list.csv')
)
pd.read_csv(
    os.path.join(reedsdir,'inputs','climate','climate_heuristics_yearfrac.csv'),
    index_col='*t',
)[sw['GSw_ClimateHeuristics']].round(3).to_csv(
    os.path.join(inputs_case,'climate_heuristics_yearfrac.csv')
)
pd.read_csv(
    os.path.join(reedsdir,'inputs','climate','climate_heuristics_finalyear.csv'),
    index_col='*parameter',
)[sw['GSw_ClimateHeuristics']].round(3).to_csv(
    os.path.join(inputs_case,'climate_heuristics_finalyear.csv')
)
pd.read_csv(
    os.path.join(reedsdir,'inputs','capacitydata','upgrade_costs_ccs_coal.csv'),
    index_col='t',
)[sw['ccs_upgrade_cost_case']].round(3).rename_axis('*t').to_csv(
    os.path.join(inputs_case,'upgrade_costs_ccs_coal.csv')
)
pd.read_csv(
    os.path.join(reedsdir,'inputs','capacitydata','upgrade_costs_ccs_gas.csv'),
    index_col='t',
)[sw['ccs_upgrade_cost_case']].round(3).rename_axis('*t').to_csv(
...
for i, row in regionFiles.iterrows():
    ...
    if filepath != filepath:
        ...
        full_path = os.path.join(reedsdir,filepath)
    ...
    if region_col == 'wide':
        ...
        if df.columns.isin(val_r_cap).any():
            df = df.loc[:,df.columns.isin(fix_cols + val_r_cap)]
    ...
    # if there is a region to region mapping set
        ...
        df = df.loc[df[r].isin(val_r_cap) & df[rr].isin(val_r_cap)]
    ...
    elif (region_col.strip('*') == 'rb_cendiv'):
        # make sure both the r is in val_r and cendiv is in val_cendiv
        df = df.loc[df['rb'].isin(val_r)]
        df = df.loc[:,df.columns.isin(["rb"] + val_cendiv)]
    ...
    # if region_col is not wide, st, or aliased..
        ...
        df = df.loc[df[region_col].isin(val_r_cap)]
...
userinputs = os.listdir(os.path.join(reedsdir,"inputs","userinput"))

for file_name in userinputs:
    full_file_name = os.path.join(reedsdir,"inputs","userinput", file_name)
...
if sw['unitdata'] == 'ABB':
    ...
    out = os.path.join(reedsdir,'inputs','capacitydata')
