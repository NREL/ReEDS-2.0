...
def compute_region_flows(flows, osprey_inputs, args):
    ...
    idx = (flows_uncapped['MW'] - abs(flows_uncapped['Val'])) > 0.01
    flows_uncapped = flows[idx].reset_index(drop=True)
    ...
    transCapacity = transCapacity.pivot(index='index', columns='path',
                                        values='MW')
    transCapacity = pd.DataFrame(
        columns=transCapacity.columns,
        data=np.tile(transCapacity.values, (args['osprey_ts_length'], 1)))

    # Get transmission loss rates in wide format
    tranLoss = osprey_inputs['tranloss'].copy()
    tranLoss['path'] = tranLoss.r + '_' + tranLoss.rr
    tranLoss['index'] = 0
    tranLoss = tranLoss.pivot(index='index', columns='path',
                              values='loss_factor')
    tranLoss = pd.DataFrame(
        columns=tranLoss.columns,
        data=np.tile(tranLoss.values, (args['osprey_ts_length'], 1)))

    # Get transmission flows in wide format
    flows['path'] = flows['r'] + '_' + flows['rr']  # add path column
    flows['index_hr'] = ((flows.d.str[1:].astype(int) - 1)*24
                         + flows.hr.str[2:].astype(int) - 1)
    flows = flows.pivot(index='index_hr', columns='path', values='Val')
    # Reindex to include unused paths
    flows = flows.reindex(index=np.arange(args['osprey_ts_length']),
                          columns=transCapacity.columns).fillna(0)

    # Apply transmission losses only to imports
    imports = pd.DataFrame(columns=flows.columns,
                           data=flows.values * (1 - tranLoss.values))

    # Get exports by region
    imports = imports.T.reset_index()
    imports = pd.merge(left=path_mapper[['rr', 'path']], right=imports,
                       on='path')
    imports = imports.groupby('rr').sum().T
    # Get imports by region
    exports = flows.copy().T.reset_index()
    exports = pd.merge(left=path_mapper[['r', 'path']], right=exports,
                       on='path')
    exports = exports.groupby('r').sum().T

    # Get the net transmission flows
    region_flows = pd.DataFrame(columns=imports.columns,
                                data=imports.values - exports.values)

    # Get the remaining transmission capacity for each path
    trancap_remain = pd.DataFrame(columns=flows.columns,
                                  data=transCapacity.values - flows.values)
    # To account for any rounding issues, force the transcap_remaining values
    # to be positive
    trancap_remain = pd.DataFrame(columns=flows.columns,
                                  data=np.clip(trancap_remain.values,
                                               a_max=None, a_min=0))

    print1('determining transmission-connected regions...')

    # Lists and counts
    bas = osprey_inputs['rfeas']['r'].tolist()
    n_bas = len(bas)
    ...
    # pXX values, because then the networkx package would connect regions
    # across time.
    rh_idx_map = pd.DataFrame(itertools.product(bas, index_hrs),
                              columns=['r', 'idx_hr'])
    rh_idx_map['rh_idx'] = (n_hours*(rh_idx_map['r'].str[1:].astype(int) - 1)
                            + rh_idx_map['idx_hr'])
    rh_idx_map['rr'] = rh_idx_map['r'].copy()  # Copy for use in merging below

    # Merge the unique rh_idx onto both the source and destination
    flows_uncapped = flows_uncapped.merge(index_hr_map, on=['d', 'hr'],
                                          how='left')
    flows_uncapped = flows_uncapped.merge(
        rh_idx_map[['r', 'idx_hr', 'rh_idx']], on=['r', 'idx_hr'], how='left')
    flows_uncapped = flows_uncapped.rename(columns={'rh_idx': 'rh_source_idx'})
    flows_uncapped = flows_uncapped.merge(
        rh_idx_map[['rr', 'idx_hr', 'rh_idx']],
    ...
    trans_region_map_subset = np.empty([n_hours*n_bas, 2])
    counter = 0
    ...
    trans_region_map_subset = trans_region_map_subset[:counter, :]
    # It isn't full if BAs didn't have uncapped transmission available to
    # anywhere
    ...
    trans_region_map_subset_df = trans_region_map_subset_df.merge(
        rh_idx_map[['rh_idx', 'r', 'idx_hr']], on='rh_idx', how='left')

    trans_region_map_df = pd.DataFrame(itertools.product(bas, index_hrs),
                                       columns=['r', 'idx_hr'])
    trans_region_map_df = trans_region_map_df.merge(
        trans_region_map_subset_df[['r', 'idx_hr', 'trans_region']],
        on=['r', 'idx_hr'], how='left')
    trans_region_map_df['trans_region_unique'] = \
        np.arange(trans_region_map_df['trans_region'].max(),
                  trans_region_map_df['trans_region'].max()
                  + len(trans_region_map_df))
    trans_region_map_df['trans_region'] = \
        np.where(trans_region_map_df['trans_region'].isnull(),
                 trans_region_map_df['trans_region_unique'],
                 trans_region_map_df['trans_region'])
    trans_region_map_df = trans_region_map_df.drop(
        columns=['trans_region_unique'])
...
def process_osprey_results(args, osprey_inputs):
    ...
    print1('reading in osprey results...')
    charging = pd.read_csv(
        os.path.join(args['data_dir'],
                     'storage_in_{}.csv'.format(args['tag'])))
    prices = pd.read_csv(
        os.path.join(args['data_dir'], 'prices_{}.csv'.format(args['tag'])))
    state_of_charge = pd.read_csv(
        os.path.join(args['data_dir'],
                     'storage_level_{}.csv'.format(args['tag'])))
    gen = pd.read_csv(
        os.path.join(args['data_dir'], 'gen_{}.csv'.format(args['tag'])))
    flows = pd.read_csv(
        os.path.join(args['data_dir'], 'flows_{}.csv'.format(args['tag'])))

    print1('processing generation results...')
    ...
    print1('processing transmission results...')
