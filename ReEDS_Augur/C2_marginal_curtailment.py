...
import numpy as np
import os
from utilities import tech_types, get_as_hourly, expand_df, \
    storage_curt_recovery, get_remaining_cycles_per_day, print1
...
def marginal_curtailment(args, marg_curt_data, osprey_results, curt_results):
    ...
    ----------
    args : Dictionary
        ReEDS Augur arguments.
    marg_curt_data : Dictionary
        Marginal curtailment input data.
    osprey_results : Dictionary
        Osprey results.
    ...
    '''
    #%%
    print1('calculating marginal curtailment...')
    ...
    d_hr = pd.read_csv(os.path.join('inputs_case', 'index_hr_map.csv'))
    d_hr = d_hr[['d', 'hr']].copy()
    d_hr['d_hr'] = d_hr['d'] + '_' + d_hr['hr']
    r_rs = marg_curt_data['r_rs'].copy()
    r_rs.set_index('rs', inplace=True)
    ts_length = len(net_load_adj)
    marg_stor_props = marg_stor_props[
        ['i', 'rte', 'duration']].drop_duplicates()
    ...
    i_resource_r = pd.merge(left=r_rs.reset_index(),
                            right=resources.rename(columns={'r': 'rs'}),
                            on='rs', how='outer')
    ...
    i_resource_r_utility = i_resource_r[i_resource_r['i'].isin(
        techs['VRE_UTILITY'])].reset_index(drop=True)
    ...
    resource_device_utility['resource_device'] = \
        resource_device_utility['resource'] + '_' \
        + resource_device_utility['device']
    ...
    # Add in H17
    curt_tran_h_h17 = curt_tran_h[curt_tran_h['h'] == 'h3'].reset_index(
        drop=True)
    curt_tran_h_h17['h'] = 'h17'
    curt_tran_h = pd.concat([curt_tran_h, curt_tran_h_h17],
    ...
    curt_reduct_tran_max['value'] = curt_reduct_tran_max['value'].round(
        args['decimals'])
    # Add in H17
    curt_reduct_tran_max_h17 = curt_reduct_tran_max[
        curt_reduct_tran_max['h'] == 'h3'].reset_index(drop=True)
    curt_reduct_tran_max_h17['h'] = 'h17'
    curt_reduct_tran_max = pd.concat([curt_reduct_tran_max,
                                      curt_reduct_tran_max_h17],
                                     sort=False).reset_index(drop=True)
    ...
    load_path_source = expand_df(
        net_load_adj_no_curt, path_mapper[['path', 'r']], 'r', 'path', ['r'])
    # Reformatting the columns to match flows
    load_path_source = load_path_source[flows.columns]

    # ability to reduce imports is the minimum of the flow along a path
    # and the load in the source region.
    imports_can_reduce = pd.DataFrame(columns=flows.columns,
                                      data=np.minimum(flows.values,
                                                      load_path_source.values))

    # expand net_load_marg to include a column for each transmission path with
    # the load of the destination region for each path
    load_path_dest = expand_df(
        net_load_adj_no_curt, path_mapper[['path', 'rr']], 'rr', 'path',
        ['rr'])
    # Reformatting the columns to match flows
    load_path_dest = load_path_dest[flows.columns]
    ...
    neighboring_load_served = imports_can_reduce.T.reset_index()
    neighboring_load_served = pd.merge(left=path_mapper,
                                       right=neighboring_load_served,
                                       on='path')
    ...
    load_path_dest = pd.DataFrame(
        columns=load_path_dest.columns,
        data=load_path_dest.values - neighboring_load_served.values)
    ...
    imports_can_reduce = pd.DataFrame(
        ...
        data=imports_can_reduce.values * (1 - tranlossrates.values))

    # scale the load of the destination region up by the transmission loss rate
    load_path_dest = pd.DataFrame(
        columns=load_path_dest.columns,
        data=load_path_dest.values / (1 - tranlossrates.values))
    ...
    exports_avail_path = pd.DataFrame(
        ...
        data=np.minimum(export_trans_cap_remain.values, load_path_dest.values))

    # group the eligible imports and exports by region
    imports_can_reduce = imports_can_reduce.T.reset_index()
    imports_can_reduce = pd.merge(left=path_mapper[['rr', 'path']],
                                  right=imports_can_reduce, on='path')
    imports_can_reduce = imports_can_reduce.groupby('rr').sum().T
    exports_avail_path = exports_avail_path.T.reset_index()
    exports_avail_path = pd.merge(left=path_mapper[['r', 'path']],
                                  right=exports_avail_path, on='path')
    exports_avail_path = exports_avail_path.groupby('r').sum().T
    ...
    load_avail_utility = pd.DataFrame(
        ...
        data=np.where(trans_region_curt, 0, net_load_adj_no_curt.values
                      + exports_avail_path.values + imports_can_reduce.values))
    ...
    load_avail_dist = pd.DataFrame(
        ...
        data=np.where(trans_region_curt, 0,
    ...
    load_avail_resource_dist = expand_df(load_avail_dist, i_resource_r_dist,
                                         'r', 'resource', ['i', 'r'])

    # expand load_avail_utility to include a profile for each utility-scale
    # resource in the "i" set
    load_avail_resource_utility = expand_df(
        load_avail_utility, i_resource_r_utility, 'r', 'resource', ['i', 'r'])
    ...
    net_load_marg = pd.DataFrame(
        columns=recf_marg.columns,
        data=(load_avail_resource.values - recf_marg.values))
    ...
    marg_curt_h = pd.concat([hdtmap_single[['h']], marg_curt], sort=False,
                            axis=1)
    marg_curt_h = marg_curt_h.groupby('h').sum()

    # get the total generation of each marginal resource before curtailment
    marg_gen_h = pd.concat([hdtmap_single[['h']], recf_marg], sort=False,
                           axis=1)
    ...
    print1('calculating potential recovery of marginal curtailment '
           + 'from marginal storage...')

    # Clip the curtailment profiles to the size of marginal storage and
    # recompute curtailment that could be recovered by storage if the marginal
    #  storage size is smaller than the marginal vre size
    if marg_stor_MW < marg_vre_mw:
        marg_curt_stor = pd.DataFrame(
            columns=marg_curt.columns,
            data=np.clip(marg_curt.values, a_min=0, a_max=marg_stor_MW))
        marg_curt_stor_h = pd.concat([hdtmap_single[['h']], marg_curt_stor],
                                     sort=False, axis=1)
    ...
    poss_batt_changes_marg = pd.DataFrame(
        columns=load_avail_resource_utility.columns,
        data=-1*np.clip(net_load_marg_utility,
                        a_min=-marg_stor_MW,
                        a_max=marg_stor_MW))

    # Get the marginal curtialment recovery rate for each marginal storage
    # technology
    ...
    for i in marg_stor_props.index:
        ...
        marg_curt_marg_stor_recovery_ratio[i] = storage_curt_recovery(
            marg_stor_MWh, marg_stor_eff, ts_length, poss_batt_changes_marg,
            hdtmap_single, marg_curt_h_utility)

    # =========================================================================
    # Repeating for marginal storage with existing curtailment
    # =========================================================================

    print1('calculating potential recovery of existing curtailment '
           + 'from marginal storage...')

    # clip net_load_adj at +/- marg_stor_MW to get the available
    # charge/discharge profile of marginal storage with existing VRE
    poss_batt_changes_margexist = pd.DataFrame(
        columns=net_load_adj.columns,
        data=-1*np.clip(net_load_adj, a_min=-marg_stor_MW, a_max=marg_stor_MW))
    ...
    # have recovered
    curt_region = pd.DataFrame(
        columns=net_load_adj.columns,
        data=np.clip(net_load_adj.values, a_min=-marg_stor_MW, a_max=0))
    # Get existing curtailment by timeslice
    curt_region_h = pd.concat([hdtmap_single[['h']], curt_region], sort=False,
                              axis=1)
    ...
    for i in marg_stor_props.index:
        ...
        exist_curt_marg_stor_recovery_ratio[i] = storage_curt_recovery(
            marg_stor_MWh, marg_stor_eff, ts_length,
            poss_batt_changes_margexist, hdtmap_single, curt_region_h)
    ...
    cap_stor_resource_device = pd.merge(
        left=resource_device_utility[['device', 'resource_device']],
    ...
    net_load_marg_resource_device_utility = expand_df(
        net_load_marg_utility,
        resource_device_utility[
            ['resource', 'device', 'r', 'resource_device']],
        'resource', 'resource_device', ['resource', 'device', 'r'])
    # reorder columns to match cap_stor_resource_device
    net_load_marg_resource_device_utility = \
        net_load_marg_resource_device_utility[
    ...
    # combination
    curt_resource_device_utility = pd.DataFrame(
        columns=net_load_marg_resource_device_utility.columns,
        data=-1*np.clip(net_load_marg_resource_device_utility.values,
                        a_min=-exist_stor_MW, a_max=0))

    # aggregate this curtailment by timeslice
    curt_resource_device_utility_h = pd.concat([hdtmap_single[['h']],
                                                curt_resource_device_utility],
                                               sort=False, axis=1)
    curt_resource_device_utility_h = \
        curt_resource_device_utility_h.groupby('h').sum()

    # get the possible storage level changes for each device for each
    # resource-device combination
    poss_batt_changes_exist = pd.DataFrame(
        columns=net_load_marg_resource_device_utility.columns,
        data=-1*np.clip(net_load_marg_resource_device_utility.values,
                        a_min=-exist_stor_MW, a_max=exist_stor_MW))

    marg_curt_exist_stor_recovery_ratio = storage_curt_recovery(
        exist_stor_MWh, exist_stor_eff, ts_length, poss_batt_changes_exist,
        hdtmap_single, curt_resource_device_utility_h)
    ...
    stor_level = get_as_hourly(stor_level, 'd_hr', 'device', 'Val',
                               net_load_adj, d_hr)

    # get the remaining cycles-per-day for storage in Osprey
    remaining_cycles_per_day = get_remaining_cycles_per_day(
        stor_level, marg_curt_exist_stor_recovery_ratio,
        resource_device_utility, d_hr, cap_stor,
        args['marg_curt_cycles_per_day'])

    # Adjust curt_recovery_ratio_exist by the cycles remaining per day
    marg_curt_exist_stor_recovery_ratio = pd.DataFrame(
        index=marg_curt_exist_stor_recovery_ratio.index,
        columns=marg_curt_exist_stor_recovery_ratio.columns,
        data=marg_curt_exist_stor_recovery_ratio.values
        * remaining_cycles_per_day.values)
    ...
    resource_device_utility.loc[resource_device_utility['i'].isin(
        techs['PV']), 'src'] = 'pv'
    resource_device_utility.loc[resource_device_utility['i'].isin(
        techs['WIND']), 'src'] = 'wind'
    i_resource_r_utility.loc[i_resource_r_utility['i'].isin(
        techs['PV']), 'src'] = 'pv'
    i_resource_r_utility.loc[i_resource_r_utility['i'].isin(
        techs['WIND']), 'src'] = 'wind'
    ...
    curt_stor = pd.merge(left=cap_stor_reeds[['i', 'v', 'r']], right=curt_stor,
                         on=['i', 'r'])
    ...
    for i in exist_curt_marg_stor_recovery_ratio.keys():
        ...
        curt_stor = pd.concat([curt_stor, temp], sort=False).reset_index(
            drop=True)
    ...
    for i in marg_curt_marg_stor_recovery_ratio.keys():
        ...
        temp = pd.merge(left=i_resource_r_utility, right=temp, on='resource',
                        how='right')
    ...
    # Add in H17
    curt_stor_h17 = curt_stor[curt_stor['h'] == 'h3'].reset_index(drop=True)
    curt_stor_h17['h'] = 'h17'
    curt_stor = pd.concat([curt_stor, curt_stor_h17], sort=False).reset_index(
        drop=True)

    # Format marginal curtailment results for ReEDS
    surplusmarginal = curt_ratio_h.reset_index().melt(id_vars='h').rename(
        columns={'variable': 'resource'})
    surplusmarginal = pd.merge(left=surplusmarginal, right=resources,
    ...
    # Remove small numbers and round results
    surplusmarginal.loc[surplusmarginal['value'] < args['min_val'],
                        'value'] = 0
    surplusmarginal['value'] = surplusmarginal['value'].round(args['decimals'])
    # Add in H17
    surplusmarginal_h17 = surplusmarginal[surplusmarginal['h'] == 'h3']
    surplusmarginal_h17 = surplusmarginal_h17.reset_index(drop=True)
    surplusmarginal_h17['h'] = 'h17'
    surplusmarginal = pd.concat([surplusmarginal, surplusmarginal_h17],
                                sort=False).reset_index(drop=True)
