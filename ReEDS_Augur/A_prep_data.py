...
def main(t, casedir):
    ...
    vre_gen_exist.index = h_dt_szn.set_index(['season','year','h','hour']).index
    ...
    vre_cf_marg.index = h_dt_szn.set_index(['season','year','h','hour']).index
    ...
    h5out['load'] = load_year.set_index(
        h_dt_szn.set_index(['season','year','h','hour']).index)
