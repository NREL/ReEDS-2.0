...
cap_wind(i,v,r)                    "--MW-- initial wind capacity"
...
cap_wind(i,v,r) = sum{t$[tcur(t)$valcap(i,v,r,t)$wind(i)], CAP.l(i,v,r,t)} ;
...
execute_unload 'reeds_server%ds%users_output%ds%%user%%ds%%runname%%ds%runs%ds%%case%%ds%augur_data%ds%reeds_data_%case%_%next_year%.gdx' avail_filt, cap_pv, cap_storage, cap_thermal, cap_trans, cap_wind, cf_hyd_filt,
                                                         ...
                                                         rfeas, routes_filt, sdbin, storage, storage_duration, storage_eff, szn, tranloss, v, cf_adj_t_filt, pvf_onm, pvf_capital, sw_sasia_trade;
